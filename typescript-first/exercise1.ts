...
// 2
const logBigger = (a: number, b:number) => {
    console.log(a > b ? a : b);
};
logBigger(1, 2);
// 3
const isEven = (n: number): string => {
    return n & 1 ? 'Odd number' : 'Even number';
};
console.log(isEven(1));
console.log(isEven(2));
// 4
const lengthOfString = (str: string): number => {
    return str.length;
};
console.log(lengthOfString('four'));
// 5
const range = (upper: number): number[] => {
    const array = [];
    for (let i = 1; i <= upper; i++) array.push(i);
    return array;
};
...
// 12
const oldestReader = (readers:Reader[]):Reader => {
    let oldest: Reader = readers[0];
    for (let i of readers) {
        if (i.age > oldest.age) oldest = i;
    };
    return oldest;
};
// 13
const oldestBook = (books: Book[]): Book => {
    let oldest: Book = books[0];
    for (let i of books) {
        if (i.year < oldest.year) oldest = i;
    };
    return oldest;
};
// test 12 and 13
